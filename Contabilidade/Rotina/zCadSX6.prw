...
/*/
User Function AltMVEST()

	Local i

	Private _cModulo := ""
	Private _cArq    := GetSrvProfString( "StartPath", "" ) + 'modulos.csv'
	Private _nIArq
	Private _cBuffer := ""
	Private _cBuffer2 := ""
	Private _aDados := {}
	...
	_cModulo := "SIGA"+cModulo//RetModName()

	_nArq	:= FT_FUse(_cArq)

	if _nArq < 0
		MsgAlert(" Não foi possível encontrar o arquivo: "+_cArq+" !")
		Return
	endif
	...
	While !FT_FEOF()
		...
		_nPosTok		:= AT(";",_cBuffer)

		_cBuffer2 := SubStr(_cBuffer,_nPosTok+1,Len(_cBuffer))

		IF _cModulo $ _cBuffer
			_aDados	:= StrTokArr(_cBuffer2,";")
		EndIF
...
Static Function fMontaTela(nOpcP, nRecP)
    ...
    DEFINE MSDIALOG oDlgEdit TITLE "Dados:" FROM 000, 000  TO nJanAltu, nJanLarg COLORS 0, 16777215 PIXEL
        ...
        If nOpcP == 2 .Or. nOpcP == 5
            ...
            oGetCon:lActive := .F.
        Else
            //Se for alteração, desabilita a Parametro.
            If nOpcP == 4
                oGetFil:lActive := .T.
                oGetPar:lActive := .F.
                oGetTip:lActive := .T.
