...
If _nOp <> 3
	If !(CTD->(DbSeek(xFilial("CTD")+"F"+ALLTRIM(SA2->A2_COD)+ALLTRIM(SA2->A2_LOJA))))
		...
		SA2->A2_XITEMCC   := "F"+ALLTRIM(SA2->A2_COD)+ALLTRIM(SA2->A2_LOJA)
	EndIF
ElseIf _nOp == 3
	If !(CTD->(DbSeek(xFilial("CTD")+"F"+ALLTRIM(M->A2_COD)+ALLTRIM(M->A2_LOJA))))
		RecLock("CTD",.T.)
		CTD->CTD_FILIAL   := xFilial("CTD") 
		CTD->CTD_ITEM	  := "F"+ALLTRIM(M->A2_COD)+ALLTRIM(M->A2_LOJA)
		CTD->CTD_CLASSE   := "2"          
		CTD->CTD_DESC01   := M->A2_NOME
		CTD->CTD_BLOQ	  := "2"    
	   	CTD->CTD_DTEXIS   := CTOD("01/01/1980")
	   	CTD->CTD_ITLP 	  := "F"+ALLTRIM(M->A2_COD)+ALLTRIM(M->A2_LOJA)
		CTD->(MsUnLock())          

		M->A2_XITEMCC   := "F"+ALLTRIM(M->A2_COD)+ALLTRIM(M->A2_LOJA)
	EndIF
Endif	
RestArea(aAreaSA2)
RestArea(aAreaCTD)
