...
PRIVATE NMEIO		:= ((NMAXWIDTH+NMINLEFT)/2)
	...
	IF SM0->(DBSEEK(CEMPANT+MV_PAR04))
		...
		CCEP     := SUBSTR(SM0->M0_CEPENT,1,5) + '-' + SUBSTR(SM0->M0_CEPCOB,6,3)
...
//--------------------------------------------------------------------------------------------------------------------------------------
	...
	_CQUERY += " 		A6_COD, A6_AGENCIA, A6_DVAGE, A6_NUMCON, A6_DVCTA, SE1NF.E1_IRRF AS IRFAT, FPZ_PROJET, FP0_XLOCAL"
	...
	_CQUERY += " 	     	F2_VALBRUT, F2_DESCONT, A1_ENDCOB, A1_BAIRROC, A1_MUNC, A1_ESTC, A1_CEPC," 
	_CQUERY += " 		    A1_ENDENT, A1_BAIRROE, A1_MUNE, A1_ESTE, A1_CEPE, A6_COD, A6_AGENCIA, A6_DVAGE, A6_NUMCON, A6_DVCTA, SE1NF.E1_IRRF, FPZ_PROJET, FP0_XLOCAL "
...
//--------------------------------------------------------------------------------------------------------------------------------------
	...
	_CQUERY += " 		  SE1NF.E1_VENCTO, F2_VALBRUT, F2_DESCONT , A2_END, A2_BAIRRO, A2_MUN, A2_EST, A2_CEP," 
	_CQUERY += " 		  A2_END, A2_BAIRRO, A2_MUN, A2_EST, A2_CEP, A6_COD, A6_AGENCIA, A6_DVAGE, A6_NUMCON, A6_DVCTA, SE1NF.E1_IRRF, FPZ_PROJET, F4_FINALID, FP0_XLOCAL"
*/
//--------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------------
		...
		NLIN :=  30
		OREPORT:SayBitmap( NLIN-5, NMINLEFT, cLogo,50,50)
...
If lMvLocBac
    _cQuery += ", FPZ_PERLOC PERLOC, FPZ_FROTA FROTAX "  // ALTERADO DE FPZ_PERLOC PERLOC
    _cQuery += ", FPA.FPA_XPLACA FROTA " // Adicionando o campo da tabela FPA apenas quando lMvLocBac = .T.
Else
//  _cQuery += ", C6_XPERLOC PERLOC, ' ' FROTA "
EndIf
...
_cQuery += "SC6.D_E_L_E_T_=' ' "
		...
		IF _CTIPO == "2"
			...
			OREPORT:SAY( NLIN-2, NMINLEFT+40, STR0024, OFONT1:OFONT ) //"DESCRIMINAùùO/ESPECIFICAùùO"
		...
		ELSE
			OREPORT:SAY( NLIN-2, NMINLEFT+10, STR0024, OFONT1:OFONT )	 //"DESCRIMINAùùO/ESPECIFICAùùO"
...
*/
		...
		OREPORT:SAY( NLIN, NMEIO+108, ALLTRIM(TRBFAT->CONTRATO), OFONT2:OFONT ) // GABRIEL 09042025
...
/*
		...
		OREPORT:SAY( NLIN-1, ((NMEIO+NMAXWIDTH)/2), "VALOR BRUTO:", OFONT1:OFONT )
		...
		OREPORT:SAY( NLIN-1, ((NMEIO+NMAXWIDTH)/2), "DESCONTO:", OFONT1:OFONT )
		OREPORT:SAY( NLIN-1, ((NMEIO+NMAXWIDTH)/2)+82, TRANSFORM(TRBFAT->F2_DESCONT,"@E 9,999,999,999.99"), OFONT2:OFONT ) // Rossana 
		NLIN +=  15 		// 580
		OREPORT:SAY( NLIN-1, ((NMEIO+NMAXWIDTH)/2), "VALOR LÕQUIDO:", OFONT1:OFONT )  //"TOTAL:" //gabriel 10042025
		...
		// --> CORRECAO DO VALOR DOS IMPOSTOS RETIDOS NA EMISSAO DO RELATORIO   (*FINAL* )
		NLIN +=  30 		// 610
		OREPORT:SAY( NLIN-1, NMINLEFT+15, "Dados do Pagamento:", OFONT1:OFONT ) //"ENDEREùO COBRANùA"
		...
		OREPORT:SAY( NLIN+30, NMINLEFT+10, "Conta: " , OFONT2:OFONT ) //"CIDADE: "###"  CEP: " + ALLTRIM(TRBFAT->A1_MUNC) + "/" + TRBFAT->A1_ESTC + STR0039 + ALLTRIM(Transform(TRBFAT->A1_CEPC, "@R 99999-99")),
